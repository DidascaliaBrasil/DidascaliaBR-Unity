...
public class ClassManager : Singleton<ClassManager>
{
    ...
    // Desliga o alarme se o objeto for destruído
    private void OnDisable()
    {
        SceneManager.sceneLoaded -= OnSceneLoaded;
    }

    // A MÁGICA ACONTECE AQUI: Quando a nova tela terminar de carregar...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        // Se viemos com a mochila cheia E não estamos mais no Menu...
        if (ClassroomData.DadosCarregados != null && scene.name != "Menu")
        {
            // 1. O Arquiteto abre a mochila e anota os dados
            SetBoysNumber(ClassroomData.DadosCarregados.numBoys);
            SetGirlsNumber(ClassroomData.DadosCarregados.numGirls);
            SetNumDeks(ClassroomData.DadosCarregados.numDesks);
            SetShape(ClassroomData.DadosCarregados.shape);
            SetRows(ClassroomData.DadosCarregados.rows);
            SetCols(ClassroomData.DadosCarregados.cols);
            SetRadius(ClassroomData.DadosCarregados.radius);

            // --> A LINHA NOVA DO SEMICÍRCULO:
            Settings.MaxDesksInSemiCircle = ClassroomData.DadosCarregados.maxDesksInSemiCircle;

            // 2. Manda construir tudo!
            GenerateClass();
            AddStudentsToDesks();
            ...
            // 3. Esvazia a mochila para não rodar duas vezes
            ClassroomData.DadosCarregados = null;
        }
    }
